...
const ActivityItem: React.FC<ActivityItemProps> = ({ activity, onDelete, onToggle, onUpdate }) => {
    ...
    const handleSave = () => {
        onUpdate({ ...activity, title: editedTitle.trim(), description: editedDescription.trim() });
        setIsEditing(false);
    };
    ...
    return (
        <div className="group relative bg-white p-4 rounded-lg shadow-sm hover:shadow-md transition-shadow">
            <div className="flex items-start space-x-6">
                <div className="w-16 text-right font-semibold text-gray-500 pt-1">{activity.time}</div>
                    ...
                    {isEditing ? (
                         <div className="space-y-2 pb-2">
                            ...
                            <div className="flex justify-end space-x-2 pt-2">
                                <button onClick={handleCancel} className="px-3 py-1 text-sm font-medium text-gray-600 bg-gray-100 rounded hover:bg-gray-200">Cancelar</button>
                                <button onClick={handleSave} className="px-3 py-1 text-sm font-medium text-white bg-cyan-500 rounded hover:bg-cyan-600">Guardar</button>
                            </div>
                        </div>
                    ) : (
...
const SuggestedActivityItem: React.FC<{
    ...
    return (
        <div className="group relative bg-white p-4 rounded-lg border-2 border-dashed border-gray-300">
            <div className="flex items-start space-x-6">
                <div className="w-16 text-right font-semibold text-gray-400 pt-1">{activity.time}</div>
                <div className="flex-1 border-l border-gray-200 pl-6 relative flex justify-between items-center">
                    ...
                    <div>
                        <h3 className="font-bold text-lg text-gray-500">{activity.title}</h3>
                    ...
                    <button
                        ...
                        aria-label={`Añadir ${activity.title}`}
                    >
...
const ActivitiesView: React.FC<ActivitiesViewProps> = ({ activities, onAddActivity, onDeleteActivity, onToggleActivity, onUpdateActivity }) => {
    const [isAddModalOpen, setIsAddModalOpen] = useState(false);

    const handleSaveActivity = (activityData: { title: string, description: string, time: string }) => {
    ...
    const futureSuggestions = SUGGESTED_ACTIVITIES.filter(suggestion => suggestion.time > currentTime);
    ...
    return (
        <div className="p-8 relative h-full bg-gray-50">
            ...
            {isAddModalOpen && <AddActivityModal onSave={handleSaveActivity} onClose={() => setIsAddModalOpen(false)} />}
        </div>
    );
};
